...
=======
SDP has been successfully applied to a variety of invasive species management problems, including optimizing prevention versus control strategies for zebra mussels (\citeproc{ref-leungOuncePreventionPound2002}{Leung et al. 2002}), designing control policies for invasive plants such as leafy spurge (\citeproc{ref-hyderEcologySocietyIntegrating2008}{Hyder et al. 2008}), identifying state-dependent strategies for spatially structured invasions (\citeproc{ref-bogichSampleEradicateCost2008}{Bogich et al. 2008}), optimizing search effort for invasive insects (\citeproc{ref-baxterOptimizingSearchStrategies2011}{Baxter and Possingham 2011}), and comparing alternative strategies across invasion stages (\citeproc{ref-hyytiainenOptimizationFrameworkAddressing2013}{Hyytiäinen et al. 2013}). More recent developments, including partially observable Markov decision processes (POMDPs), allow decision-making under imperfect detection, while connections with reinforcement learning highlight promising avenues for scaling these approaches to complex, data-rich systems (e.g. Haubrock et al. (\citeproc{ref-Haubrock2025}{2025})) {[}Abby: Is this paper an example of reinforcement learning for invasive species management?{]}. {[}Cassie: For POMDP, the Waring et al 2023 MEE paper could be a good one to point to{]}

Despite these advances, the practical implementation of SDP and related approaches remains limited in applied animal ecology. This gap is partly due to the perceived complexity of these methods and the lack of accessible, step-by-step guidance for translating ecological problems into operational decision models.

In this paper, we aim to bridge this gap by providing a practical guide to the use of stochastic dynamic programming and its extensions for invasive species management. Using the coypu (\emph{Myocastor coypus}) as a running example, we illustrate how to formulate decision problems, implement models, and derive optimal management strategies across a range of ecological contexts, from single populations to metapopulations. We first introduce the core concepts of structured decision making and stochastic dynamic programming through a minimal, fully transparent example. We then develop a series of worked tutorials of increasing complexity, including spatial dynamics, imperfect detection using partially observable Markov decision processes (POMDPs), and model uncertainty addressed through adaptive management. Throughout, we provide reproducible implementations in \texttt{R} to facilitate uptake and adaptation by practitioners. {[} Cassie: I feel like we should keep using the acronymns here (i.e. SDP, POMDP) though if we use SDM too it might become confusing.. {]}

{[} Cassie: Given that we have the sections below to detail SDM, SDP and AM in more detail, should we maybe focus the intro a little more on why we would want to manage invasive species, challenges in making the management decisions and just briefly introduce the SDM, SDP and AM? I haven't read the 'How to' style papers for JAE so not sure how much redundancy they usually have {]}
...
% We've mentioned some applications of SDP in the intro

We do not want to rewrite Lucile's paper\ldots{}

\subsection{Structured decision making}\label{structured-decision-making}

{[}Abby: Maybe a good place to describe structured decision making and how the methods we present are mostly optimization methods useful for evaluating the consequences of actions (i.e., we're not addressing the aspects of SDM such as problem framing, eliciting objectives; we're instead assuming this has already been done){]} {[} Cassie: I've briefly tried to address these points below; references need work {]}

(\citeproc{ref-conroy2013decision}{Conroy and Peterson 2013}, \citeproc{ref-StructuredDecisionMaking2020}{Runge et al. 2020})

Structured decision making (SDM) provides a framework for decision makers to approach complex decision problems by clearly establishing the decision context, formulating clear and measurable objectives, identifying the available management alternatives (actions), identifying the key uncertainties that might impact the management outcomes, and managing trade-offs and constraints (Gregory et al. 2012). The SDM framework thus breaks complex decisions into smaller components, producing decisions that are more transparent and robust than decisions made without a structured approach.

In this paper, we present some of the methods that can be used to solve complex decision problems formulated using an SDM process by evaluating the consequences of alternative management actions in the context of the objectives, uncertainty and trade-offs. Naturally, that means we are assuming that the decision context has already been defined, the objectives elicited, and alternative actions identified, before implementing the below approaches. {[} Cassie: Here could be a good point to direct people to the Hemming et al 2021 paper {]}
...
{[}Mettre ce tableau avant l'équation, pour insister d'abord sur l'intuition.{]}
...
Solving the finite-horizon decision problem {[}ça va un peu vite, et il faut introduire la figure{]} yields the optimal value function \(V_t(s)\), which represents the expected cumulative reward (i.e., negative total cost) obtained when starting from state \(s\) at time \(t\) and following the optimal policy thereafter. Values are more negative when the invasion is established, reflecting higher expected damages. Moreover, values increase (become less negative) as the horizon shortens because fewer future costs can accumulate. {[}Détailler le calcul?{]}
...
{[}Rappeller les différentes étapes, reprendre le papier de Lulu.{]}
...
=======
A major limitation of SDP is the ``curse of dimensionality'', whereby the size of the state and action space grows rapidly with system complexity. This can restrict the number of state variables, spatial resolution, or management options that can be included in a model. In practice, this often requires simplifying assumptions, state aggregation, or the use of simulation-based approaches to approximate optimal policies. {[} Cassie: Discuss the heuristic sampling (HS) method that approximates the optimal policy for any starting state from Nicol & Chades 2011? {]}
>>>>>>> f4c1cf266f4c93cfd210221d10a040591838257f
...
=======
Although SDP can be implemented using simple or expert-based models, its relevance depends on the availability and quality of data describing system dynamics, management effects, and costs. In data-limited contexts, SDP may still provide valuable qualitative insights into decision structure and trade-offs, but quantitative recommendations should be interpreted with caution. With large state spaces, and particularly for POMDPs, model outputs can become unmanageable and uninterpretable, making it necessary to reduce the state (and observation) space to the minimum necessary to capture the state dynamics. Alternatively, Ferrer-Mestres et al. (2020) leverages AI to present an approach that starts with all states of an SDP and iteratively aggregates states to a smaller number while minimising the performance impacts.

{[} Cassie: could we split data reqs and interpretation? We could then discuss a little the process of getting the data for objectives and managemet alternatives (i.e. it's mostly through expert elicitation not from scaping data from papers) {]}
>>>>>>> f4c1cf266f4c93cfd210221d10a040591838257f
...
\subsection{Accounting for observation error with POMDP (Cassie)}\label{acounting-for-observation-error-w-pomdp-cassie}
...
POMDPs differ from MDPs by including an observation state (O), observation function (Z), and initial belief state (\(b_0\)). The observation state includes the set of observations perceived by the manager, while the observation function represents the conditional probability of a manager observing o' given that action a lead to state s'. In cases where state uncertainty cannot be reduced, the observation function is independent of the actions taken, and thus dependent only on state s'. The initial belief represents the believed state of the system at a given time.

Because the states are themselves unobservable, the state of the system must be tracked with belief states. At any point in time, the actual (unobservable) state of the system influences the immediate returns, state transitions, and observations, but do not influence actions. The belief state is updated based on observations, which subsequently inform the action(s) to be selected. Lastly, actions drive the state transitions, rewards, and may influence the observations. Some may find it helpful to think of POMDPs as an optimal control framework for hidden Markov models (HMMs), where the true system state is latent and must be inferred from observation states (Chades et al 2021).
...
Going back to a simple example in invasive species management, imagine we have \ldots. {[}Occupancy model with uncertain detectability{]}
...
The model can be solved using standard POMDP algorithms. In the code, the same problem is implemented with the \texttt{pomdp} package and with \texttt{sarsop}. The solution is represented by a set of \(\alpha\)-vectors. For any belief state \(b\), the value of each candidate policy vector is obtained by a dot product: {[} Cassie: I think it would be better to place the description of SARSOPs and describe the alternative solvers here rather than later on in the active AM section {]}
...
\section{Conclusion (all)}\label{conclusion-all}
...
Chenery, A. M., L. Blackwood, C. M. Tortorelli, and others. 2020. \href{https://doi.org/10.1002/ecs2.3011}{Reducing uncertainty in species management: Forecasting secondary spread with expert opinion and mechanistic models}. Ecosphere 11:e03011.
...
Gregory, R., L. Failing, M. Harstone, G. Long, T. McDaniels, and D. Ohlson. 2012. \href{https://onlinelibrary.wiley.com/doi/book/10.1002/9781444398557}{Structured decision making: A practical guide to environmental management choices}. Wiley-Blackwell, Chichester, West Sussex.

\bibitem[\citeproctext]{ref-haight2010optimal}
Haight, R. G., and S. Polasky. 2010. \href{https://doi.org/10.1016/j.reseneeco.2010.04.005}{Optimal control of an invasive species with imperfect information about the level of infestation}. Resource and Energy Economics 32:519--533.
=======
...
Hemming, V., A. E. Camaclang, M. S. Adams, M. Burgman, K. Carbeck, J. Carwardine, I. Chadès, L. Chalifour, S. J. Converse, L. N. K. Davidson, G. E. Garrard, R. Finn, J. R. Fleri, J. Huard, H. J. Mayfield, E. M. Madden, I. Naujokaitis-Lewis, H. P. Possingham, L. Rumpff, M. C. Runge, D. Stewart, V. J. D. Tulloch, T. Walshe, and T. G. Martin. 2022. \href{https://doi.org/10.1111/cobi.13868}{An introduction to decision science for conservation}. Conservation Biology 36:e13868.
...
Moore, A. L., and M. A. McCarthy. 2016. \href{https://doi.org/10.1111/2041-210X.12564}{Optimizing ecological survey effort over space and time}. Methods in Ecology and Evolution 7:891--899.

\bibitem[\citeproctext]{ref-nicol2011beyond}
Nicol, S., and I. Chadès. 2011. \href{https://doi.org/10.1111/j.2041-210X.2010.00069.x}{Beyond stochastic dynamic programming: A heuristic sampling method for optimizing conservation decisions in very large state spaces}. Methods in Ecology and Evolution 2:221--228.

\bibitem[\citeproctext]{ref-nicol2012which}
Nicol, S., and I. Chadès. 2012. \href{https://doi.org/10.1371/journal.pone.0028993}{Which states matter? An application of an intelligent discretization method to solve a continuous {POMDP} in conservation biology}. PLoS ONE 7:e28993.
...
Regan, H. M., M. Colyvan, and M. A. Burgman. 2002. \href{https://doi.org/10.1890/1051-0761(2002)012\%5B0618:ATATOU\%5D2.0.CO;2}{A taxonomy and treatment of uncertainty for ecology and conservation biology}. Ecological Applications 12:618--628.
...
Schapaugh, A. W., and A. J. Tyre. 2012. \href{https://doi.org/10.1111/j.2041-210X.2012.00242.x}{A simple method for dealing with large state spaces}. Methods in Ecology and Evolution 3:954--961.
