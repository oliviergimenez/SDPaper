...
where \(R(s,a)\) is the immediate reward, \(P_a(s,s')\) the transition probability, \(\beta\) the discount factor, and \(V_{t+1}(s')\) the optimal future value (see Figure \ref{fig:fig-bellman}).

\begin{figure}

{\centering \includegraphics[width=0.9\linewidth]{bellman-fig-v2} 
...
The Bellman recursion is solved by backward induction. At the final time step, no future rewards remain, so \(V_T(s)=0\) for every state. The algorithm then proceeds backwards through time. At each state and time step, it evaluates every available action, combines the immediate reward with the expected future value, and selects the action that maximizes the total expected reward.

Solving the finite-horizon decision problem yields the optimal value function \(V_t(s)\), which represents the expected cumulative reward (i.e., negative total cost) obtained when starting from state \(s\) at time \(t\) and following the optimal policy thereafter:
...
Values are more negative when the invasion is established, reflecting higher expected damages. Moreover, values increase (become less negative) as the horizon shortens because fewer future costs can accumulate.
...
\subsection{4.1 Single population}\label{single-population}
...
\subsection{4.2 Metapopulation}\label{metapopulation}
...
Occupancy is updated stochastically based on local abundance and spatial context (Figures \ref{fig:fig-metapop}a and \ref{fig:fig-metapop}b). When a site is occupied, extinction probability decreases with abundance according to
...
Management decisions are evaluated through a cost function that balances ecological damages and intervention effort (Figure \ref{fig:fig-metapop}c). At each time step, total cost is defined as
...
The resulting policy is illustrated in Figure \ref{fig:fig-metapop}d.~When few sites are occupied and populations remain small, limited intervention is often sufficient because damages are modest and extinction may occur naturally. As more sites become occupied or local abundances increase, stronger control becomes optimal in order to reduce future spread and prevent escalating impacts. Temporal variation in recommended actions arises because the finite planning horizon reduces the benefit of aggressive intervention late in the time period.

Comparing forward simulations under the optimal policy with simulations under constant strategies highlights the value of adaptive management (Figure \ref{fig:fig-metapop}e). The optimal strategy typically reduces cumulative costs by applying effort strategically in time rather than maintaining a fixed level of control.

Spatial trajectories further illustrate how coordinated management alters invasion dynamics across the network, with reductions in abundance and occupancy emerging progressively across sites (Figure \ref{fig:fig-metapop}f).
...
\subsection{4.3 Acounting for observation error with POMDP}\label{acounting-for-observation-error-with-pomdp}
...
\subsection{4.4 Reducing model uncertainty with adaptive management}\label{reducing-model-uncertainty-with-adaptive-management}
